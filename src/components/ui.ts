...
export function setupUI(k: KAPLAYCtx): UIHandles {
    ...
    const skillSize = 64;
    const skillBg = k.add([k.rect(skillSize, skillSize, { radius: 8 }), k.pos(k.width() - skillSize - 20, k.height() - skillSize - 20), k.color(24, 24, 28), k.outline(3, k.rgb(255, 255, 255)), k.fixed(), k.z(1200), { id: "ui-skill-bg" }]);
    const skillKey = k.add([k.text("Q", { size: 28 }), k.pos(skillBg.pos.x + skillSize / 2 - 8, skillBg.pos.y + skillSize - 34), k.color(220, 220, 220), k.fixed(), k.z(1202), { id: "ui-skill-key" }]);
    const cdText = k.add([k.text("", { size: 22 }), k.pos(skillBg.pos.x + 10, skillBg.pos.y + 10), k.color(255, 255, 255), k.fixed(), k.z(1203), { id: "ui-skill-cd-text" }]);
    const cdMask = k.add([k.rect(skillSize, skillSize), k.pos(skillBg.pos.x, skillBg.pos.y), k.color(0, 0, 0), k.opacity(0.55), k.fixed(), k.z(1201), { id: "ui-skill-cd-mask" }]);
    ...
    k.onResize(positionSkillUI); positionSkillUI();

    k.onUpdate(() => {
        const skillId = gameState.skills.skill1;
        if (!skillId || !skillsRegistry[skillId]) {
            skillBg.hidden = true; skillKey.hidden = true; cdText.hidden = true; cdMask.hidden = true;
        } else {
            skillBg.hidden = false; skillKey.hidden = false;
            ...
            if (remaining > 0) {
                ...
                cdText.hidden = false; cdMask.hidden = false;
                ...
                cdText.hidden = true; cdMask.hidden = true;
            }
        }
        skillOverlay.update();
    });
    ...
    return {
        ...
        updateGold: (amount) => { topBar.updateGold(amount); },
