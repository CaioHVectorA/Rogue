...
export function spawnGreen(
  k: KAPLAYCtx,
  target: GameObj,
  arena: { x: number; y: number; w: number; h: number },
) {
  const p = ENEMY_PRESETS.green;
  const e = commonEnemy(k, {
    target,
    arenaBounds: arena,
    size: p.size,
    color: p.color,
